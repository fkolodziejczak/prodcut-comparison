...
function deepFreeze(target) {
  ...
  Object.getOwnPropertyNames(target).forEach((prop) => {
    ...
    if (hasOwnProperty(target, prop) && (targetIsFunction ? prop !== "caller" && prop !== "callee" && prop !== "arguments" : true)) {
      ...
      if ((isObjectLike(propValue) || typeof propValue === "function") && !Object.isFrozen(propValue)) {
...
function hasOwnProperty(target, propertyName) {
  return isObjectLike(target) ? Object.prototype.hasOwnProperty.call(target, propertyName) : false;
}
...
function patchState(stateSource, ...updaters) {
  stateSource[STATE_SOURCE].update((currentState) => updaters.reduce((nextState, updater) => freezeInDevMode(__spreadValues(__spreadValues({}, nextState), typeof updater === "function" ? updater(nextState) : updater)), currentState));
...
function withState(stateOrFactory) {
  return (store) => {
    ...
    store[STATE_SOURCE].update((currentState) => freezeInDevMode(__spreadValues(__spreadValues({}, currentState), state)));
